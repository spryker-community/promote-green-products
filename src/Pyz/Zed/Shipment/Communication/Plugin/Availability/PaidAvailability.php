 ...
 */
class PaidAvailability extends AbstractPlugin implements ShipmentMethodAvailabilityPluginInterface
{
    use LoggerTrait;
    ...
    {
        $isPaidProductShipping = false;
        
        foreach($quoteTransfer->getItems() as $itemTransfer) {
            /**@var \Generated\Shared\Transfer\ItemTransfer $itemTransfer */
            $concreteAttributes = $itemTransfer->getConcreteAttributes();
            $isPaidProductShipping = !isset($concreteAttributes['co2']);
            if ($isPaidProductShipping) {
                break;
            }
        }

        return $isPaidProductShipping;
    }
}
